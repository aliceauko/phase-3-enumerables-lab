...
def print_spicy_foods(spicy_foods)
  spicy_foods.map do |food|
    ...
    puts "#{food[:name]} (#{food[:cuisine]}) | Heat Level: #{heats}"
  end
...
def sort_by_heat(spicy_foods)
  spicy_foods.sort do |firstfood, secondfood|
    firstfood[:heat_level] <=> secondfood[:heat_level]
  end
end
...
def print_spiciest_foods(spicy_foods)
  spiciest_food = spicy_foods.filter {|f| f[:heat_level]>5}
